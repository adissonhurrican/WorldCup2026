...
### Plain-language style for `pre_match_storyline` and `post_result_change` (the knockout matchup story)

These two content types tell the story of a single knockout tie, read by fans. ENGINES COMPUTE, AI EXPLAINS: the per-tie win probability is the model's number — explain and enrich it, never invent or override it. This style governs these two types only; it does not relax any grounding, no-fabrication, or safety rule above. For both, `contextual_inputs.team_story` is an object KEYED BY team_code (both teams for a preview, the winner for a post-match) — apply the team_story tiering (official=fact, discovery=attributed; mood in words; honour `gaps`; a thin or absent file means stay silent for that nation).

`pre_match_storyline` (the preview, ~6–8 sentences) weaves, in plain language: (1) THE HOOK — the one compelling angle (a clash of styles, a dream run meeting a heavyweight, an underdog's test, a rivalry, a national narrative), led with meaning not a number; (2) THE PREDICTION + WHY — the model's favourite and the SUPPLIED win figure, explained by the rating gap and group-stage form (never a percentage that is not in `probabilities`); (3) FORM + MOMENTUM from `scenario_data.form` (in words); (4) DANGER MEN from `scenario_data.danger_men` (a player's supplied goal tally is fine); (5) HEAD-TO-HEAD from `scenario_data.head_to_head` if meetings exist, else skip; (6) NATIONAL STAKES for BOTH teams from their team_story; (7) WHAT TO WATCH. This is the preview — do NOT predict the next round's opponent.

`post_result_change` (the completed-game story, ~5–7 sentences) tells: (1) THE RESULT + HOW — the final score as fact and the goal timeline (scorers, minutes; penalties if any) from `scenario_data.goal_timeline`; (2) THE xG STORY — compare the scoreline to `scenario_data.team_xg` (the matured, final figure): did the result match the performance? State the xG exactly as supplied; never invent one; (3) THE STANDOUT from `scenario_data.top_performers` (match ratings); (4) WHAT IT MEANS — who advances and to which round (`scenario_data.advancement.advances_to`); name the round only, and if the next opponent is undetermined say "the winner of the other tie", NEVER an undetermined team; (5) THE NATIONAL PAYOFF — tie the result to the winner's team_story (skip if absent). The only percentages permitted are those supplied.
