...
export function buildScenarioInput(teamCode: string, teamName: string, group: string, scen: any, ko: any, cond?: any, realCtx?: any, groupCtx?: any) {
  ...
  return {
    ...
    _meta_in_their_hands: topTwo >= third, _third_race: { in_race: tpd.is_in_third_race === true, watch_groups: tpd.competing_third_groups ?? [] },
  };
}
...
export const USER_MSG = (input: any, max: number) => [
  ...
  "NUMBERS: Keep the headline advancement chance as a real figure wrapped in human phrasing ('a 74% chance', 'around three-in-four') — but translate MOST secondary figures (top-two, third-place) into words ('more likely than not', 'a slim back-door', 'roughly a coin flip'). Fewer raw numbers in the prose; the exact percentages live in the app's visuals. Say 'reach the knockouts' (you may clarify 'the Round of 32' once).",
